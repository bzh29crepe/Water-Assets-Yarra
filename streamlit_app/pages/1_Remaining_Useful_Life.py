...
def preprocess_for_prediction(df):
    """Add missing computed features required by the model."""
    current_year = datetime.now().year

    # Compute 'age' if not present
...
The data shown comes from `yarra_assets_unknown.csv` and the RUL values are generated by the trained model.
""")

# ---- Load data and model ----
df = load_assets()
df = preprocess_for_prediction(df)  # Add 'age' column
model = load_model()

# ---- Generate predictions ----
df = predict_rul(df, model)

df.loc[df["Predicted_RUL"] == 0, "status"] = "Decommissioned"

df.loc[(df["Predicted_RUL"] != 0) & (df["status"] == "Decommissioned"), "status"] = "Active"
...
st.dataframe(filtered_df[[
    "asset_id", "asset_name", "asset_type", "material", "installation_year",
    "Predicted_RUL", "zone", "status", "location", "age"
